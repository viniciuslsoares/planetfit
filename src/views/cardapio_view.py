...
def render_cardapio():
    ...
    with col_config2:
        st.info(
            "💡 **Regra de Distribuição:** 20% no Café da Manhã e o restante dividido igualmente entre as demais refeições."
        )
    ...
    for i in range(n_refeicoes):
        ...
        with st.expander(
            f"{nome_ref} - Meta: {meta_atual['kcal']:.0f} kcal", expanded=(i == 0)
        ):
            st.write(
                f"🎯 **Metas da Refeição:** {meta_atual['prot']:.1f}g Proteína | {meta_atual['carb']:.1f}g Carboidrato"
            )
            ...
            c_sel = c2.selectbox(
                "🍚 Carboidrato", df_total["alimento"], key=f"c_sel_{i}"
            )
            v_sel = c3.selectbox(
                "🥦 Vegetal/Volume", df_total["alimento"], key=f"v_sel_{i}"
            )

            if st.button(
                f"Gerar Sugestão Prática para {nome_ref}", key=f"btn_calc_{i}"
            ):
                ...
                if resultado:
                    ...
                    r3.metric(
                        f"{v_sel}", f"{gv:.0f}g", help="Volume ajustado para saciedade."
                    )
                    ...
                    st.caption(
                        "Valores finais após arredondamento das porções para medidas práticas:"
                    )
                    ...
                    st.markdown(
                        f":{cor_diff}[*Variação de {diff_kcal:+.0f} kcal em relação à meta teórica.*]"
                    )

                else:
                    st.error(
                        "⚠️ Não foi possível encontrar um ajuste prático. Tente trocar os alimentos selecionados."
                    )
