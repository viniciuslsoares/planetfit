...
def render_receitas(df_taco):
    ...
    with st.expander("➕ Adicionar Ingrediente à Receita", expanded=True):
        ...
        if busca_ing:
            matches = df_base[df_base['alimento'].str.contains(busca_ing, case=False, na=False)]
            escolha = st.selectbox("Selecione o alimento:", matches['alimento'].tolist())
            
            col_q, col_btn = st.columns([3, 1])
            quantidade = col_q.number_input("Quantidade (g)", min_value=1.0, value=100.0)
